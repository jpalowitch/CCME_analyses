...
make_ccm <- function (param_list,
  ...
  cv <- param_list$cv
  hv <- param_list$hv
  deg_scaler <- param_list$deg_scaler
  
  if (!is.null(input_cv)) {
    cv <- input_cv
  }
  
  if (!is.null(deg_scaler)) {
    k <- deg_scaler * k
    max_k <- deg_scaler * max_k
  }
  ...
  if (is.null(input_degrees)) {
    dVec <- power_law(N, tau_1, max_set = max_k,
                      mean_set = k)
    d1 <- sort(dVec, decreasing = TRUE)[1]
    d2 <- sort(dVec, decreasing = TRUE)[2]
    dT <- sum(dVec)
    counter <- 0
    while (d1 * d2 / dT > 1) {
      counter <- counter + 1
      dVec <- power_law(N, tau_1, max_set = max_k - counter,
                        mean_set = k)
      d1 <- sort(dVec, decreasing = TRUE)[1]
      d2 <- sort(dVec, decreasing = TRUE)[2]
      dT <- sum(dVec)      
  ...
  if (is.null(input_strengths)) {
    ...
    sVec <- input_strengths
  }
  
  
  node1 <- rep(1:N, each = N)
  node2 <- rep(1:N, N)
  keepers <- which(node1 < node2)
  node1 <- node1[keepers]
  node2 <- node2[keepers]
  
  
  # Edges
  pVec <- dVec[node1] * dVec[node2] / sum(dVec)
  edges <- rbinom(length(pVec), 1, pVec)
  
  edgeList <- data.frame("node1" = node1[edges == 1],
                         "node2" = node2[edges == 1],
                         "weight" = rep(0, sum(edges)))
  
  # Weights
  wVec <- sVec[edgeList$node1] * sVec[edgeList$node2] / sum(sVec)
  wVec <- wVec / pVec[edges == 1]
  edgeList$weight <- sapply(wVec, wtSim)
...
make_sbm <- function (param_list,
  ...
  if (N == 0) {
    ...
    if (hv == 0) {
      ...
      cat("err: need some nodes in either comms or bg\n")
      return(NULL) 
      
    } else {
      
      sbm <- make_ccm(param_list)
      return(sbm)
  ...
  deg_scaler <- param_list$deg_scaler
  c_scaler <- param_list$c_scaler
  if (!is.null(c_scaler)) {
    max_c <- c_scaler * max_c
    min_c <- c_scaler * min_c
  }
  if (!is.null(deg_scaler)) {
    k <- deg_scaler * k
    max_k <- deg_scaler * max_k
  ...
  # Computing number of memberships
  if (on == 0) {
    n_m <- N
  } else {
    n_m <- N - on + on * om
  ...
  if (is.null(input_truth)) {
    ...
    OL_nodes <- sample(1:N, on, replace = FALSE)
    n_memberships <- rep(1, N)
    n_memberships[OL_nodes] <- om
...
sbm_summary <- function (sbm) { 
  ...
  phi <- sbm$phi
  phi_scale <- sbm$phi_scale
  psi <- sbm$psi
  Estrengths <- sbm$Estrengths
  Edegrees <- sbm$Edegrees
  truth <- sbm$truth
  
  hv <- param_list$hv
  N <- param_list$N
  n_c <- length(truth$communities)
  s2n_e <- param_list$s2n_e
  s2n_w <- param_list$s2n_w
  n <- hv + N
